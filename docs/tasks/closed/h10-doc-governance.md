...
| `docs/tasks/closed/h10-quality-gates-matrix.md` | Quality Owner | new guardrail command, removed command, command contract changes |
| `docs/tasks/closed/h10-failure-triage.md` | Quality Owner | new recurring CI/guardrail failure class, changed error message/pattern, new required CI job |
| `docs/tasks/closed/h10-release-hygiene.md` | Release Owner | required checks set changes, rollback policy changes, merge gate policy changes |
| `docs/tasks/closed/h09-ci-gates.md` | Quality Owner | workflow/job/required-check changes in `.github/workflows/**` |
| `docs/tasks/closed/h02-dependency-rules.md` | App Architecture Owner | dependency policy changes, allow/deny matrix updates, new architectural enforcement rules |
...
7. A new top-level file is added under `docs/` or an existing active doc is moved to/from `docs/archive/**`.

## 4.1 Active vs Archived Docs

- Top-level `docs/*.md` files are the active source-of-truth set and must be registered in `docs/README.md`.
- `docs/archive/historical/**` stores former source-of-truth notes and closeout evidence.
- `docs/archive/superseded/**` stores documents replaced by newer active contracts.
- `docs/archive/exploratory/**` stores alternative or abandoned plans retained for context only.
- Archived docs are non-authoritative unless an active doc or archived task handoff points to them explicitly.
...
2. Weekly quick scan (lightweight):
   ...
   - validate required-check names still match `docs/tasks/closed/h09-ci-gates.md`,
   - validate H10 docs cross-links remain valid,
   - run `npm run verify-docs-governance`.
