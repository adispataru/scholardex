(function () {
  ...
  function parseRows(tbody) {
    ...
    for (var i = 0; i < children.length; i++) {
      var row = children[i];
      if (!row.classList.contains('js-activity-row')) continue;
  ...
  function boot() {
    var root = document.getElementById('activities-dashboard-v2');
    if (!root) return;

    var tbody = document.getElementById('activities-main-body');
    var emptyState = document.getElementById('activities-empty-state');
    var countLabel = document.getElementById('activities-count');

    var searchInput = document.getElementById('activities-search');
    var minScoreInput = document.getElementById('activities-min-score');
    var quarterSelect = document.getElementById('activities-quarter');
    var typeSelect = document.getElementById('activities-type');
    var sortSelect = document.getElementById('activities-sort');
    var clearBtn = document.getElementById('activities-clear');
    ...
    function render() {
      ...
      countLabel.textContent = filtered.length + ' / ' + rows.length + ' activities';
    }
