(function () {
  ...
  function renderRows(items) {
    ...
    const html = items.map(function (item) {
      ...
      return '<tr>' +
        '<td><a href="/admin/scopus/authors/edit/' + id + '">' + escapeHtml(item.name) + '</a></td>' +
        '<td>' + affiliations + '</td>' +
        '</tr>';
    }).join('');
  ...
  async function fetchPage() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Unexpected error while loading authors.');
      }
  ...
  function initialize() {
    if (!els.afid || !els.search || !els.sort || !els.direction || !els.size || !els.loading || !els.error || !els.empty || !els.tableBody || !els.pageInfo || !els.totalInfo || !els.prev || !els.next) {
      return;
    }
