(function () {
  ...
  const els = {
    ...
    direction: document.getElementById('events-direction'),
    size: document.getElementById('events-size'),
    ...
    totalInfo: document.getElementById('events-total-info'),
    prev: document.getElementById('events-prev'),
  ...
  function renderRows(items) {
    ...
    els.tableBody.innerHTML = items.map(function (item) {
      const name = item.name ? escapeHtml(item.name) : '<span class="app-table__cell--muted">—</span>';
      const rank = item.rankLabel ? escapeHtml(item.rankLabel) : '<span class="app-table__cell--muted">—</span>';
      const domain = item.domainId ? escapeHtml(item.domainId) : '<span class="app-table__cell--muted">—</span>';

      return '<tr>' +
        '<td>' + name + '</td>' +
        '<td>' + rank + '</td>' +
        '<td class="app-table__cell--secondary">' + domain + '</td>' +
        '</tr>';
    }).join('');
  }
  ...
  function buildUrl() {
    ...
    return '/events/data?' + params.toString();
  }
  ...
  async function fetchPage() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Unexpected error while loading events.');
      }
