...
class AdminSourceLinkControllerSecurityContractTest {
    ...
    @Test
    void nonAdminCannotAccessSourceLinksPage() throws Exception {
    ...
    @Test
    void nonAdminCannotRunReconcile() throws Exception {
        mockMvc.perform(post("/admin/source-links/reconcile")
                        .param("confirmation", "RECONCILE")
                        .with(csrf())
                        .with(user("researcher@uvt.ro").authorities(new SimpleGrantedAuthority("RESEARCHER"))))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/custom-error?error=403"));
