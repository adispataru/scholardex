...
- [ ] `H52` Indicator / scoring / formula flow — v1.
  Goal: replace the (OutputType × Strategy) enum cross-product, the `Map<String,Object>` open-bag `Score.extra`, the per-researcher `MVEL.eval(string)` reparse, the hand-written strategy-to-service `if/else if` ladder, and the hardcoded `LAST_YEAR = 2023` with a type-safe `IndicatorKind` hierarchy, typed `BaseScore`/`FormulaContext`/`Provenance`, compile-cached + sandboxed MVEL formulas with per-kind variable contracts, a `Map<Strategy, ScoringService>` registry, and data-driven year caps — while keeping numerical equality against the 2,463-row `userIndicatorResults` history.
  Deliverable: sealed `IndicatorKind` (`Publications(role)`, `Citations(excludeSelf)`, `Activity(type)`, `GenericCount`, `GenericActivity`) replacing `Indicator.outputType` + `Indicator.scoringStrategy`; `YearRangeSpec` / `ScoreYearRangeSpec` / `Selector` value types; per-strategy variable contract enforced at indicator-save; compile-once formulas with `formulaHash` as cache identity; `Map<ScoringStrategy, ScoringService>` registry; deletion of `Indicator.Type.CITATIONS`, the `min` MVEL rewrite, `Score.errors`/`details`/`extra`, the `IY` grammar branch on `yearRange`, and the duplicate `Mate_S (copy)`; `Indicator.domain` retained as optional filter with runtime semantics wired; replay-equality regression harness.
  Exit criteria: every existing indicator produces numerically identical results to the historical cache (1e‑9 tolerance, exceptions itemized); `MVEL.eval(string, ...)` does not appear in any hot path; rejecting an indicator save whose formula references an undeclared variable is a tested behavior; `LAST_YEAR` is grep-clean; the pair `(outputType, scoringStrategy)` no longer appears in the codebase.
  Dependency: none direct; planning doc at `docs/tasks/active/h52-indicator-scoring-v1.md`. `ComputerScienceConferenceScoringService` (1,251 LOC) stays on the new `ScoringService` interface untouched internally and is a separate follow-up.
